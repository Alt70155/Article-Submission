class PostValidator < ActiveModel::Validator
  ...
  def validate(post)
    raise ActiveRecord::RecordInvalid 'img_files_in_articleプロパティが定義されていません。' unless defied?(post.img_files_in_article)
    ...
    post.errors[:img_files_in_article] << '画像ファイルが添付できる上限数を超えています。' if post.img_files_in_article > DEFAULT_MAX_IMG_CNT
